...
class BulkRNASeq_VVProtocol(VVProtocol):
    ...
    def validate_dataset(self) -> Dict[BulkRNASeqDataset, List[Flag]]:
        ...
        if STAGE.Demultiplexed in self._stages_loaded:
            flags[self.dataset].append(
                self.run_check(self.dataset_metadata_0001, dataset=self.dataset)
            )
            flags[self.dataset].append(
                self.run_check(self.dataset_rawReads_0001, dataset=self.dataset)
            )
        if STAGE.Reads_PreProcessed in self._stages_loaded:
            flags[self.dataset].append(
                self.run_check(self.dataset_trimReads_0001, dataset=self.dataset)
            )
        if STAGE.GenomeAligned in self._stages_loaded:
            flags[self.dataset].append(
                self.run_check(self.dataset_genomeAlignments_0001, dataset=self.dataset)
            )
        if STAGE.RSeQCAnalysis in self._stages_loaded:
            flags[self.dataset].append(
                self.run_check(self.dataset_rseqcAnalysis_0001, dataset=self.dataset)
            )
    ...
    def validate_samples(self) -> Dict[BulkRNASeqSample, List[Flag]]:
        ...
        for sample in self.dataset.samples.values():
            if STAGE.Demultiplexed in self._stages_loaded:
                flags[sample].append(
                    self.run_check(self.sample_rawReads_0001, sample=sample)
                )
            if STAGE.Reads_PreProcessed in self._stages_loaded:
                flags[sample].append(
                    self.run_check(self.sample_trimReads_0001, sample=sample)
                )
        return flags
            ...
            match component:
                case RawReadsComponent():
                    if STAGE.Demultiplexed in self._stages_loaded:
                        flags[component].append(
                            self.run_check(
                                self.component_rawReads_0001, component=component
                            )
                        )
                case TrimReadsComponent():
                    if STAGE.Reads_PreProcessed in self._stages_loaded:
                        flags[component].append(
                            self.run_check(
                                self.component_trimReads_0001, component=component
                            )
                        )
                ...
                case GenomeAlignments():
                    if STAGE.GenomeAligned in self._stages_loaded:
                        flags[component].append(
                            self.run_check(
                                self.component_genomeAlignments_0001,
                                component=component,
                            )
                        )
                case RSeQCAnalysis():
