...
{attachments_str}
    ...
    def __extract_mqcData(self, mqc_target: MQCTargetDict):
        ...
        """
        assert len(mqc_target['mqc_modules']), "Only one module per data asset currently supported"
...
Is Part of DataSystem: 
    ...
    def getMQCDataFrame(self, sample_component: str, mqc_module: str, mqc_plot: str = None) -> Union[pd.DataFrame, List[str]]:
        ...
        has_component = {sample.name:getattr(sample, sample_component, False) for sample in self.samples.values()}
        assert all(has_component.values()), f"At least one sample does not have the requested component ({sample_component})!: {has_component}"

        # check if all samples have the requested mqc module
        has_module = {sample.name:getattr(sample, sample_component).mqcData.get(mqc_module) for sample in self.samples.values()}
        assert all(has_module.values()), f"At least one sample does not have the requested multiQC module ({mqc_module})!: {has_module}"
        ...
        has_plot = {sample.name:isinstance(getattr(sample, sample_component).mqcData.get(mqc_module)["Plots"].get(mqc_plot), pd.DataFrame) for sample in self.samples.values()}
